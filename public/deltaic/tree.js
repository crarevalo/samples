jQuery(document).ready(function(){
  var url = "/deltaic/tree";
  var params = {"union_id" : jQuery("#union_id").val()};
  jQuery.get(url, params, function(result){
    var tree_breadth = result.tree_breadth;
    var tree_depth = result.tree_depth;
    var adjustedWidth = (+tree_depth) * 360;
    width = adjustedWidth;
    var adjustedHeight = (+tree_breadth) * 20;
    height = adjustedHeight;
    tree = d3.layout.tree().size([height, width]);
    diagonal = d3.svg.diagonal().projection(function(d) { return [d.y, d.x]; });
    svg = d3.select("svg").attr("width", width + margin.right + margin.left).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");

    tree_data = result.tree_data;
    nodes = tree.nodes(tree_data);
...
var margin = {top: 20, right: 200, bottom: 20, left: 200}, width = 1800 - margin.right - margin.left, height = 3000 - margin.top - margin.bottom;
...
function update(source) {
  ...
  nodeEnter.append("text").attr("x", function(d) { return d.children || d._children ? -10 : 10; }).attr("dy", ".35em").attr("text-anchor", function(d) { return d.children || d._children ? "end" : "start"; }).text(function(d) { return d.name; }).style("fill-opacity", 1e-6);
